...
| Handoffs  | `BookingChecklist`, `BookingChecklistStep`, `BookingChecklistPhoto`, `BookingChecklistLink`         | independent dropoff/pickup flows, private photos, completed-by timestamps, and a single active hashed link per flow               |
...
`PendingBookingUserEmail` is an email-normalized booking association used by handoff sharing. On signup or sign-in, matching pending rows create the booking-member association and, when needed, the corresponding team membership before the pending record is removed.
